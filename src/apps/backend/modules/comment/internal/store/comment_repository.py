...
COMMENT_VALIDATION_SCHEMA = {
    "$jsonSchema": {
        "bsonType": "object",
        "required": ["account_id", "task_id", "content", "active", "created_at", "updated_at"],
        "properties": {
...
class CommentRepository(ApplicationRepository):
    ...
    def on_init_collection(cls, collection: Collection) -> bool:
        ...
        collection.create_index(
            [("active", 1), ("account_id", 1), ("task_id", 1)],
            name="active_account_task_index",
        ...
        try:
            ...
            if e.code == 26:
                ...
                Logger.error(message=f"OperationFailure for collection comments: {e.details}")
        return True